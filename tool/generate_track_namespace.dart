...
//
// The script scans `proto/common/events/v1/*.proto`, discovers every
// `message <X>Properties` block that carries an explicit
// `option (common.events.v1.kind)` annotation, filters to mobile-applicable
// platforms via `option (common.events.v1.platforms)`, and rewrites:
//
...
String _renderTrackNamespace(List<_Event> events) {
  ...
  buf.writeln('// discouraged-path detector) is preserved verbatim by the');
  buf.writeln(
...
String _renderMethod(_Event event) {
  ...
  buf.writeln('  /// logged at DEBUG level.');
