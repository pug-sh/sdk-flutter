...
| --- | --------------------------------------------------------------------------------------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------- |
| 1   | Screen/page-view auto-tracking                                                          | `page_view` auto-emitted on init + history nav    | Only `app_open`/`app_close`; no `NavigatorObserver`, no `page_view`/`screen_view`              | High                                      |
| 2   | Screen/route context on events                                                          | `$url`, `$referrer`, `$pageTitle` auto-properties | No current-screen/route name stamped on events                                                 | Low–Medium (tied to #1)                   |
